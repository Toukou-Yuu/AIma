"""Tests for LLMPolicy Protocol compliance and dummy model integration."""
...
class TestLLMPolicyProtocol:
    ...
    def test_llm_policy_has_name(self) -> None:
        ...
        spec = _make_agent_spec(response='{"kind":"draw","seat":0}')
        policy = LLMPolicy(policy_id="test", spec=spec, seed=42)
        assert policy.name == "llm"

    def test_llm_policy_has_policy_id(self) -> None:
        """LLMPolicy has 'policy_id' attribute."""
        spec = _make_agent_spec(response='{"kind":"draw","seat":0}')
        policy = LLMPolicy(policy_id="test_policy_001", spec=spec, seed=42)
...
class TestLLMPolicyWithDummyBackend:
    """AC1: dummy model backend + LLMPolicy outputs legal action."""
    ...
    def test_dummy_backend_ignores_input(self) -> None:
        """Configured dummy response controls LLMPolicy output."""
        ctx = _make_decision_context()
        ...
        fallback_spec = _make_agent_spec(response='{"kind":"tsumo","seat":0}')
        fallback_policy = LLMPolicy(policy_id="fallback", spec=fallback_spec, seed=42)
        decision2 = fallback_policy.decide(ctx)
        # Should use fallback
...
class TestLLMPolicyFallbackBehavior:
    ...
    def test_parse_failed_recorded(self) -> None:
        ...
        spec = _make_agent_spec(response="not json at all")
        policy = LLMPolicy(policy_id="test", spec=spec, seed=42)
        ctx = _make_decision_context()
    ...
    def test_fallback_used_in_trace(self) -> None:
        ...
        spec = _make_agent_spec(response="invalid json")
        policy = LLMPolicy(policy_id="test", spec=spec, seed=42)
        ctx = _make_decision_context()
    ...
    def test_first_legal_fallback_triggers(self) -> None:
        ...
        spec = _make_agent_spec(fallback="first_legal", response="invalid json")
        policy = LLMPolicy(policy_id="test", spec=spec, seed=42)
        ctx = _make_decision_context()
...
class TestDummyBackend:
    ...
    def test_dummy_backend_configurable_response(self) -> None:
        ...
        backend1 = DummyBackend(
            ModelSpec(backend="dummy", extra={"response": "response 1"})
        )
        backend2 = DummyBackend(
            ModelSpec(backend="dummy", extra={"response": "response 2"})
        )
    ...
    def test_dummy_backend_response_setter(self) -> None:
        ...
        backend = DummyBackend(ModelSpec(backend="dummy", extra={"response": "initial"}))
    ...
    def test_dummy_backend_complete_returns_response(self) -> None:
        ...
        backend = DummyBackend(
            ModelSpec(backend="dummy", extra={"response": "test response"})
        )
